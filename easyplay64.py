"""
EasyPlay v64 - Accessible Media Player for Raspberry Pi 5.

Changes from v62 (don't wake TVs that are off):
  - cec_startup() now queries the TV's CEC power state before sending
    the "Image View On" / "Active Source" wake-up commands. If the TV
    is in standby (off), startup skips the wake step and remains a
    silent passenger on the bus. The user can still wake the TV later
    by pressing On/Off on the remote (which triggers cec_tv_on_and_select_pi).
  - Designed for the nightly-reboot.timer use case: the Pi reboots at
    03:00, easyplay.service auto-launches, but the TV stays off.
  - Also fixes the general case where the user reboots the Pi while
    the TV is off — previously easyplay would wake the TV every time.
...
def cec_query_tv_power():
    """Query the TV's CEC power state.

    Returns one of: 'on', 'standby', 'transition', 'unknown'.
    Used at startup so we don't wake a TV that's intentionally off
    (e.g. after the nightly 03:00 reboot).
    """
    try:
        if not shutil.which("cec-client"): return "unknown"
        result = subprocess.run(
            "echo 'pow 0' | cec-client -s -d 1",
            shell=True, timeout=8, capture_output=True, text=True
        )
        out = (result.stdout or "").lower()
        # cec-client prints e.g. "power status: standby" / "power status: on"
        # or "power status: in transition from on to standby"
        for line in out.splitlines():
            line = line.strip()
            if not line.startswith("power status"): continue
            if "standby" in line and "transition" not in line: return "standby"
            if "transition" in line: return "transition"
            if line.endswith(": on") or " on " in line: return "on"
        return "unknown"
    except Exception as e:
        _log(f"CEC pow query error: {e}", e)
        return "unknown"
...
def cec_startup():
    ...
    v64: query TV power state first. If TV is in standby we skip the
    wake-up entirely so a 3am cron reboot doesn't turn the TV on. The
    user's On/Off button still wakes the TV later via cec_tv_on_and_select_pi().
    """
    if not _cfg_get("cec_enabled", True): return
    def _worker():
        try:
            ...
            # ── v64: bail out quietly if the TV is off ────────────────────
            tv_power = cec_query_tv_power()
            if tv_power == "standby":
                _log("CEC startup: TV is in standby — skipping wake (v64)")
                return
            # If TV is on or unknown, proceed with the normal wake sequence.
            brand_key = _resolve_brand(get_tv_brand())
            profile = TV_BRANDS.get(brand_key, TV_BRANDS["generic"])
            delay = profile["delay_ms"] / 1000.0
