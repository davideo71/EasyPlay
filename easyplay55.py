"""
EasyPlay v55 - Accessible Media Player for Raspberry Pi 5.
...
# === Filename Cleaning ===
_CLEAN_PATTERNS = [
    r"\s*\[(?:[A-Z0-9]+(?:\.[A-Z0-9]+)*)\]", r"\s*[\(\[]\d{4}(?:-\d{4})?[\)\]]",
    r"[\.\s]\d{4}(?=[\.\s]|$)", r"[\.\s]\d+bit(?=[\.\s]|$)",
    r"\s*[\(\[]?\d{3,4}p[\)\]]?", r"\s*[\(\[]?(?:4K|8K|UHD|HDR|DV|SDR)[\)\]]?",
    r"\s*[\(\[]?[2-7]\.1[\)\]]?", r"\s*[\(\[]?2\.0[\)\]]?",
    r"\s*[\(\[]?[xhHX]\.?26[45][\)\]]?", r"\s*[\(\[]?(?:HEVC|VP9|AV1|XviD|DivX)[\)\]]?",
    r"\s*[\(\[]?(?:BluRay|BRRip|BDRip|WEBRip|WEB-DL|WEB|DVDRip|DVD|HDTV|SDTV|AMZN|NF)[\)\]]?",
    r"[\.\s](?:DDP|DD\+?|AC3|AAC|DTS(?:-HD)?(?:\.MA)?|FLAC|MP3|Atmos|TrueHD|OPUS|OGG|VORBIS|PCM|LPCM|EAC3|E-AC-3)(?:\d{1,2}\.?\d?)?(?=[\.\s\-]|$)",
    r"\s*[\(\[](?:DDP|DD\+?|AC3|AAC|DTS|FLAC|MP3|Atmos|TrueHD|OPUS|OGG|VORBIS|PCM|LPCM|EAC3|E-AC-3)[\)\]]",
    r"[\.\s](?:MP4|MKV|AVI)(?=[\.\s]|$)", r"[\.\s](?:IMAX)(?=[\.\s]|$)",
    r"\s*[\-\.](?:[A-Z0-9]{3,}|[A-Z][a-z][A-Za-z0-9]+)(?<!S\d{2})$",
    r"\s*[\(\[]?(?:PROPER|REPACK|INTERNAL|LIMITED|UNRATED|EXTENDED|DC|REMASTERED)[\)\]]?",
]
...
def find_videos_in_folder(folder):
    ...
    return [f for f in sorted(folder.iterdir())
            if f.is_file() and not _is_hidden(f) and f.suffix.lower() in VIDEO_EXTS and not _is_junk_video(f)]
...
def _ble_uart_thread(addr):
    ...
    def notification_handler(sender, data):
        try:
            ...
            if char:
                print(f"[BLE] button received: {char!r}", flush=True)
        ...
        for attempt in range(1, 4):
            ...
            try:
                ...
                if "InProgress" in str(e) or "NotReady" in str(e):
                    _log(f"BLE: BlueZ busy during scan (attempt {attempt}/3) — resetting")
...
def play_video_embedded(screen, path, start_sec, show_seek_overlay=True,
    ...
    try:
        ...
        frame_surface = pygame.Surface((_vmem_w, _vmem_h), 0, 32)
        ...
        while running:
            ...
            # ── Render frame from VLC buffer ──
            if _frame_ready and _frame_buf is not None:
                _frame_ready = False
                try:
                    raw = bytes(_frame_buf)
                    frame_surface = pygame.image.frombuffer(raw, (_vmem_w, _vmem_h), "BGRA")
                except Exception: pass
            # ── Blit video frame (letterboxed to preserve aspect ratio) ──
            screen.fill((0, 0, 0))
            # Try to get native aspect ratio if we don't have it yet
            ...
            if video_aspect is not None:
                # Letterbox: fit native aspect ratio within screen
                screen_aspect = w / h
                if video_aspect > screen_aspect:
                    # Video is wider than screen → pillarbox top/bottom
                    dst_w = w
                    dst_h = int(w / video_aspect)
                else:
                    # Video is taller than screen → letterbox left/right
                    dst_h = h
                    dst_w = int(h * video_aspect)
                dst_x = (w - dst_w) // 2
                dst_y = (h - dst_h) // 2
                scaled = pygame.transform.scale(frame_surface, (dst_w, dst_h))
                screen.blit(scaled, (dst_x, dst_y))
            else:
                # Fallback: stretch to fill (aspect unknown yet)
                if frame_surface.get_size() != (w, h):
                    screen.blit(pygame.transform.scale(frame_surface, (w, h)), (0, 0))
                else:
...
def main():
    ...
    load_cover_thumbs(items, state.cover_cache, screen)
    _log(f"Loaded {len(state.cover_cache)} covers, {len(items)} items")
    ...
    while state.running:
        ...
        # Run at 60fps during animation/transitions, drop to 15fps when idle to save CPU
