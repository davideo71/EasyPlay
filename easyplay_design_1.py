"""
EasyPlay design_1 - Accessible Media Player for Raspberry Pi 5.

Design goals for a user with cognitive disabilities:
  - At any moment, there is ONE obvious thing to do.
  - Same button always does the same thing. Avoid mode-dependent meanings.
  - Nothing on screen that the user doesn't need. Debug / setup / status
    chrome stays out of view during normal use.
  - Every button press gets an instant visual response.
  - Caregiver complexity (setup, BT pairing, keymap) stays hidden behind
    the long-press setup menu.

Changes from v60:
  - D1: Center carousel slot enlarged (~2.5× outer width). Title text moves
    directly under the center cover instead of a bottom banner, so the
    focus target and its label are visually one unit.
  - D3: Six-color palette applied consistently across menus and overlays.
  - D4: Explicit type scale (display / heading / body / meta) via make_fonts().
  - D6: Covers show two simple badges: progress bar across the bottom for
    in-progress items, a small green check badge top-right for finished
    items. Nothing else.
  - I4: Translucent "Continue: [title] ▲" banner at top of carousel for
    ~5 seconds on launch when a resumable item exists.
  - I7: Small warning dot bottom-right ONLY when the BLE remote is
    disconnected for >10 s. Invisible during normal use.
  - P1: Carousel draws immediately with placeholder gradient tiles;
    real thumbnails swap in as they load.
...
# ── Design palette (design_1) ───────────────────────────────────────────────
# Six colors used everywhere so the UI feels cohesive and calm.
class Palette:
    BG          = (10, 10, 15)          # near-black background
    BG_RAISED   = (20, 20, 28)          # cards, overlays, menus
    ACCENT      = (118, 75, 162)        # violet (app identity)
    ACCENT_HI   = (102, 126, 234)       # lighter violet for emphasis
    TEXT        = (245, 245, 250)       # primary text
    TEXT_DIM    = (140, 140, 155)       # captions, non-selected labels
    SUCCESS     = (80, 180, 120)        # finished / watched
    WARNING     = (220, 120, 60)        # BLE disconnect, errors
    PROGRESS    = (220, 220, 230)       # in-progress bar fill

# ── Typography scale ────────────────────────────────────────────────────────
# Call make_fonts(h) once screen height is known to instantiate.
def make_fonts(screen_h):
    """Return a dict of pygame Fonts scaled to the current screen height."""
    # Size each tier relative to screen height so 720p / 1080p / 4K all look right.
    scale = screen_h / 1080.0
    return {
        "display": pygame.font.SysFont("Helvetica", int(64 * scale), bold=True),
        "heading": pygame.font.SysFont("Helvetica", int(36 * scale), bold=True),
        "body":    pygame.font.SysFont("Helvetica", int(22 * scale)),
        "meta":    pygame.font.SysFont("Helvetica", int(16 * scale)),
    }

# ── BLE status ──────────────────────────────────────────────────────────────
# Set by the BLE thread; read by the main loop for the warning dot (I7).
_ble_status = {
    "connected": False,
    "last_connected_at": 0.0,  # monotonic time of last connect
    "last_disconnect_at": 0.0, # monotonic time of last disconnect
}
        ...
        while _ble_thread_running:
            try:
                ...
                async with BleakClient(device,
                    ...
                    _ble_status["last_connected_at"] = time.monotonic()
                    ...
                    if _ble_thread_running:
                        ...
                        _ble_status["last_disconnect_at"] = time.monotonic()
                        needs_reset = True
...
def cec_shutdown():
    """Release active source on exit. Never powers off the TV."""
    if not _cfg_get("cec_enabled", True): return
    cec_send("is")
...
def compute_layout(w, h, aspect=FRAME_ASPECT, center_scale=1.45, outer_scale=0.55, height_mult=1.0):
...
class AppState:
    ...
    # ── design_1 fields ────────────────────────────────────────────────
    fonts: dict = field(default_factory=dict)         # display/heading/body/meta
    resume_item_idx: int | None = None                # carousel index of resumable item
    resume_banner_until: float = 0.0                   # monotonic time to hide banner
    ble_last_connected: float = 0.0                    # mirrored from _ble_status
...
def load_cover_thumbs_async(items, cache, screen, on_each=None):
    """design_1 (P1): Load covers on a background thread.

    The caller renders with placeholders immediately; covers swap in as
    they arrive. Call on_each(cover_key) after each successful load so
    the UI can flag itself dirty.
    """
    def worker():
        for item in items:
            if item.cover_path and item.cover_path.exists():
                try:
                    ...
                    # .convert(screen) must happen on the main thread because
                    # it touches the display surface. We skip it here; the
                    # first blit will convert lazily. On pygame-SDL2 this is
                    # safe (just slightly slower than pre-converted surfaces).
                    cache[key] = surf
                    if on_each:
                        try: on_each(key)
                        except Exception: pass
...
def placeholder_tile(w, h, seed_int):
    """Return a calm gradient-filled surface for a missing cover thumbnail."""
    surf = pygame.Surface((max(1, w), max(1, h)))
    # Deterministic muted color per item so tiles don't all look identical
    # and the user still gets a stable visual shape per slot.
    import colorsys
    hue = (seed_int * 0.6180339887) % 1.0  # golden ratio distribution
...
def main():
    ...
    info = pygame.display.Info(); w = max(800, info.current_w); h = max(600, info.current_h)
    ...
    # design_1: explicit type scale
    fonts = make_fonts(h)
    # design_1: find resumable item for I4 banner
    _resume_idx = None
    _resume_ratio = 0.0
    for idx, it in enumerate(items):
        if it.video_path and not it.is_series:
            try:
                ps = str(it.video_path.resolve())
                r = get_progress_ratio(ps)
                if 0.01 < r < 0.95 and r > _resume_ratio and not get_progress_completed(ps):
                    _resume_idx = idx
                    _resume_ratio = r
            except Exception:
                pass
    state = AppState(screen=screen, w=w, h=h, font=font, items=items,
        ...
        fonts=fonts,
        resume_item_idx=_resume_idx,
        resume_banner_until=time.monotonic() + 5.0 if _resume_idx is not None else 0.0)
    # design_1 (P1): async cover loading so the carousel renders immediately.
    # Each cover marks the state dirty as it arrives.
    def _on_cover_loaded(_key):
        state.render_dirty = True
    load_cover_thumbs_async(items, state.cover_cache, screen, on_each=_on_cover_loaded)
        ...
        try:
            ...
            if state.in_picker:
                ...
                for i in range(N_SLOTS):
                    ...
                    # design_1 (P1): placeholder tile while the real cover
                    # is still being loaded on the background thread.
                    if not is_series and cover_key and cover_key not in state.cover_cache and fw > 1 and fh > 1:
                        placeholder = placeholder_tile(fw, fh, idx)
                    ...
                    # ── design_1 badges (D6) ─────────────────────────────
                    # Slim progress bar across the bottom of the cover for
                    # in-progress items. Small green check in the top-right
                    # corner for completed items. No other badges.
                    if item.video_path and not is_series:
                        path_str = str(item.video_path.resolve())
                        completed = get_progress_completed(path_str)
                        ratio = 0.0 if completed else get_progress_ratio(path_str)
                        if ratio > 0:
                            # 6px bar at the very bottom of the cover
                            pb_h = max(4, fh // 120)
                            pb_w = max(1, int(fw * ratio))
                            pygame.draw.rect(screen, (0, 0, 0, 160),
                                             (x, y + fh - pb_h, fw, pb_h))
                            pygame.draw.rect(screen, Palette.PROGRESS,
                                             (x, y + fh - pb_h, pb_w, pb_h))
                        if completed:
                            # Small green circle with a check inside, top-right corner
                            badge_r = max(12, fh // 22)
                            bx = x + fw - badge_r - 6
                            by = y + badge_r + 6
                            pygame.draw.circle(screen, Palette.SUCCESS, (bx, by), badge_r)
                            pygame.draw.circle(screen, (255, 255, 255), (bx, by), badge_r, 2)
                            # Check mark — three line segments forming a ✓
                            cw = badge_r * 0.9
                            ch = badge_r * 0.9
                    ...
                    # ── Title: ONLY under the center cover (D1) ─────────
                    # Non-center slots show no label at all — reduces visual
                    # noise and ensures there's exactly one thing to read.
                    if i == CENTER_SLOT and item.name:
                        title_font = state.fonts["display"] if state.fonts else font
                        title_surf = title_font.render(item.name, True, Palette.TEXT)
                        # Soft drop shadow so the title reads on any cover variant
                        shadow = title_font.render(item.name, True, (0, 0, 0))
                        tx = rect.centerx - title_surf.get_width() // 2
                        ty = rect.bottom + 18  # 18px breathing room under cover
                        # Fit within screen bounds
                        if tx < 10: tx = 10
                        if tx + title_surf.get_width() > w - 10:
                            tx = w - 10 - title_surf.get_width()
                        screen.blit(shadow, (tx + 2, ty + 2))
                        screen.blit(title_surf, (tx, ty))

                # ── Resume banner (I4) ──────────────────────────────────────
                # Shown at the top of the carousel for ~5 seconds after launch
                # when there's a partially-watched item. Fades out quietly.
                if state.resume_item_idx is not None and time.monotonic() < state.resume_banner_until:
                    state.render_dirty = True  # keep animating the fade
                    remaining = state.resume_banner_until - time.monotonic()
                    alpha = int(min(220, remaining * 70))  # fade out in last ~3s
                    if alpha > 0 and 0 <= state.resume_item_idx < len(state.items):
                        ri = state.items[state.resume_item_idx]
                        if ri.name:
                            banner_font = state.fonts.get("heading", font)
                            msg = f"Continue: {ri.name}  ▲"
                            if len(msg) > 60: msg = msg[:57] + "…  ▲"
                            msg_surf = banner_font.render(msg, True, Palette.TEXT)
                            pad_x, pad_y = 32, 14
                            bw = msg_surf.get_width() + pad_x * 2
                            bh = msg_surf.get_height() + pad_y * 2
                            bx = (w - bw) // 2; by = 40
                            bg = pygame.Surface((bw, bh), pygame.SRCALPHA)
                            bg.fill((*Palette.ACCENT, alpha))
                            screen.blit(bg, (bx, by))
                            msg_surf.set_alpha(min(255, alpha + 35))
                            screen.blit(msg_surf, (bx + pad_x, by + pad_y))

                # ── BLE disconnect warning dot (I7) ─────────────────────────
                # Only shown when the BLE remote has been disconnected for >10s.
                # Caregiver can see at a glance that the remote is offline;
                # the user sees nothing during normal operation.
                now_m = time.monotonic()
                if (not _ble_status.get("connected", False)
                        and _ble_status.get("last_disconnect_at", 0) > 0
                        and (now_m - _ble_status.get("last_connected_at", now_m)) > 10.0
                        and (now_m - _ble_status.get("last_disconnect_at", 0)) > 10.0):
                    state.render_dirty = True  # pulse redraws
                    r = 10
                    cx, cy_dot = w - 28, h - 28
                    pulse = 0.6 + 0.4 * abs(math.sin(now_m * 2))
                    dot_color = tuple(int(c * pulse) for c in Palette.WARNING)
                    pygame.draw.circle(screen, dot_color, (cx, cy_dot), r)
                    pygame.draw.circle(screen, (0, 0, 0), (cx, cy_dot), r, 1)
